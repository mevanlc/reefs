...
pub fn parse_document(path: &Path, source: &str) -> Result<KdlDocument> {
    source
        .parse::<KdlDocument>()
        .map_err(|error| eyre!("{}", format_parse_error(path, source, &error)))
...
mod tests {
    ...
    #[test]
    fn formats_kdl_parse_error_with_line_column_and_snippet() {
        let source = "name \"ok\"\nbad 1.\n";
        let error = source.parse::<KdlDocument>().unwrap_err();
        let formatted = format_parse_error(Path::new("bad.kdl"), source, &error);

        assert!(formatted.contains("bad.kdl:2:5:"));
        assert!(formatted.contains("2 | bad 1."));
